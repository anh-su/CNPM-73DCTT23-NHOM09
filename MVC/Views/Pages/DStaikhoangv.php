...
<body>
  <h4 class="mb-2">ĐĂNG KÝ TÀI KHOẢN GIẢNG VIÊN</h4>
  <hr class="custom-hr">

  <!-- Form Đăng Ký -->
  <div class="form-container">
    <form method="post" action="http://localhost/Doan/DStaikhoangv/themmoi">
      <div class="form-row">
      ...
      <div class="form-group">
        <button style="background-color: #26a69a;" type="submit" class="btn btn-primary" name="btnThemGiangvien">Đăng Ký Tài Khoản</button>
      </div>
    </form>
  </div>
  ...
  <form method="post" action="http://localhost/Doan/DStaikhoangv/timkiemgv">
    <br>
    <hr class="custom-hr">
    ...
    <div class="form-group" style="margin-left: 1cm;">
        <label for="myID_Giangvien">ID Giảng Viên: </label>
        <input type="text" class="form-control1" id="myID_Giangvien" placeholder="" name="txtID_Giangvien" value="<?php if (isset($data['ID_Giangvien'])) echo $data['ID_Giangvien'] ?>">
    </div>
    <div class="form-group">
    <button style="margin-left: 1cm; background-color: #26a69a;" type="submit" class="btn btn-primary" name="btnTimkiemgv">Tìm Kiếm</button>
    </div>
    <div class="form-group">
    ...
    <table class="table table-striped">
        ...
        <?php if (!empty($data['danhSachTaiKhoangv'])) : 
                     $i = 0; ?>
            <?php foreach ($data['danhSachTaiKhoangv'] as $DStaikhoangv) : ?>
            <tr>
                        
                <td><?php  echo (++$i) ?></td>
                <td><?php echo $DStaikhoangv['ID_TK'] ?></td>
                <td><?php echo $DStaikhoangv['Tendangnhap'] ?></td>
                <td><?php echo $DStaikhoangv['Matkhau'] ?></td>
                <td><?php echo $DStaikhoangv['Vaitro'] ?></td>
                <td><?php echo $DStaikhoangv['ID_Giangvien'] ?></td>
                <td>
                    <a href="#" class="edit-link" data-ID_TK="<?php echo $DStaikhoangv['ID_TK']; ?>" data-Tendangnhap="<?php echo $DStaikhoangv['Tendangnhap']; ?>" data-Matkhau="<?php echo $DStaikhoangv['Matkhau']; ?>" data-Vaitro="<?php echo $DStaikhoangv['Vaitro']; ?>" data-ID_Giangvien="<?php echo $DStaikhoangv['ID_Giangvien']; ?>" data-toggle="modal" data-target="#editModal">
                        <img src="http://localhost/Doan/Public/Pictures/edit.gif" alt="Edit">
                    </a>
                    <a href="http://localhost/Doan/DStaikhoangv/xoagv/<?php echo $DStaikhoangv['ID_TK'] ?>">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form method="post" action="http://localhost/Doan/DStaikhoangv/capnhatgv">
                    <div class="container-fluid mt-3">
                        ...
                        <div class="form-group col-sm-6">
                            <label for="modalID_Giangvien">ID Giảng Viên</label>
                            <input type="text" class="form-control" id="modalID_Giangvien" name="txtID_Giangvien" readonly>
                        </div>
                        </div>
...
$(document).ready(function() {
    ...
    $('.edit-link').click(function() {
        ...
        var ID_Giangvien = $(this).data('id_giangvien');

        // Kiểm tra trong console
        console.log(ID_TK, Tendangnhap, Matkhau, Vaitro, ID_Giangvien);

        $('#modalID_TK').val(ID_TK);
        $('#modalTendangnhap').val(Tendangnhap);
        $('#modalMatkhau').val(Matkhau);
        $('#modalVaitro').val(Vaitro);
        $('#modalID_Giangvien').val(ID_Giangvien);
    });
});
